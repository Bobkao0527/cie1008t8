...
def safe_load_image(filename, width, height):
    ...
    search_dirs = [BASE_DIR, ORIGINAL_CWD, ".", "assets", os.path.join("assets", "Background")]
            ...
            try:
                for f in os.listdir(sd):
                    if clean_str(f) == target_clean:
                        return pygame.transform.smoothscale(pygame.image.load(os.path.join(sd, f)).convert_alpha(), (width, height))
            except: pass
    # 保底生成
...
# 你的實際視窗
WIDTH, HEIGHT = 1024, 768  # 依當前選擇
screen = pygame.display.set_mode((WIDTH, HEIGHT))

# 核心渲染虛擬畫布 (所有的 blit 和 draw 都要改畫在 canvas 上)
    ...
    if Background == "Map":
        for i in range(23):
            for j in range(23):
                ...
                if (i,j) == (Pr,Pc):
                    canvas.blit.rect(screen, Color, ((WIDTH / 160) + (j * WIDTH / 120), (HEIGHT / 120) + (i * HEIGHT / 90), WIDTH / 120, HEIGHT / 90))
                    canvas.blit.rect(screen, (0,0,0), ((WIDTH / 160) + (j * WIDTH / 120), (HEIGHT / 120) + (i * HEIGHT / 90), WIDTH / 120, HEIGHT / 90), int(max(HEIGHT / 300,1)))
                elif MAP[i,j] != 0:
                    canvas.blit.rect(screen, Color, ((WIDTH / 160) + (j * WIDTH / 120), (HEIGHT / 120) + (i * HEIGHT / 90), WIDTH / 120, HEIGHT / 90))
...
def draw_card(card, x, y):
    card_name = card["name"]
    canvas.blit(CardImage[card_name], (x, y))

def draw_event_card(card, x, y):
    card_name = card["name"]
    canvas.blit(EventCardImage[card_name], (x, y))
...
def Anim_RoomCenter2():
    ...
    canvas.blit.rect(screen, (255, 215, 0), rec, int(max(HEIGHT / 60, 1)))
...
def Anim_RoomCenter3():
    ...
    if tick <= 30:
        ...
        canvas.blit(Allblack, (0, 0))
        if tick == 30:
            Background = "Loading"
    elif tick <= 60:
        canvas.blit(recimg, (0,0))
        Allblack.set_alpha((60-tick)*8.5)
        canvas.blit(Allblack, (0, 0))
    elif tick <= 180:
        canvas.blit(recimg, (0,0))
        canvas.blit(gun, recg)
        canvas.blit(cube, recc)
    elif tick <= 210:
        canvas.blit(recimg, (0,0))
...
def Anim_RoomCenter8():
    ...
    rec.center = (WIDTH // 2, HEIGHT // 2)
    canvas.blit.rect(canvas, (0, 255, 0), rec, int(max(HEIGHT / 60, 1)))
...
def Anim_GameFire():
    ...
    rec.center = (WIDTH // 2, HEIGHT // 2)
    canvas.blit.rect(canvas, (255, 0, 0), rec, int(max(HEIGHT / 70, 1)))
...
while running:
    ...
    if Background == "StartWeapon":
        ...
        #小地圖
        canvas.blit.circle(screen, (255, 255, 255), (WIDTH * 0.41 / 4 , HEIGHT * 0.41 / 3),WIDTH / 10)
        miniMap()
        #道具卡
        draw_text(
        ...
        if SelectedWeapon is not None:
            ...
            for i in range(3):
                ...
                if is_lit:
                    canvas.blit.rect(screen, (255, 215, 0), box_rect, border_radius=5) 
                    canvas.blit.rect(screen, (255, 255, 255), box_rect, 2, border_radius=5)
                else:
                    canvas.blit.rect(screen, (60, 60, 60), box_rect, border_radius=5)
                    canvas.blit.rect(screen, (120, 120, 120), box_rect, 2, border_radius=5)
    ...
    Animation()
        ...
        screen.fill((20, 0, 0))
        draw_text("你被擊敗了！", int(WIDTH * 0.5), int(HEIGHT * 0.32), font_big, (255, 80, 80))
        draw_text("重新遊戲請按 P 鍵", int(WIDTH * 0.5), int(HEIGHT * 0.48), font_mid, (255, 255, 255))

    # =================================================================
    # 🌟 貼在這裡：滑鼠光標最上層無損貼合渲染 Pipeline
    # =================================================================
    # 1. 取得滑鼠在 1024x768 虛擬畫布中的邏輯座標
    mx, my = pygame.mouse.get_pos()
    logical_mx = int(mx * (1024 / WIDTH))
    logical_my = int(my * (768 / HEIGHT))
    logical_mouse_pos = (logical_mx, logical_my)

    # 2. 偵測按鈕 Hover 狀態與點擊，更新並繪製彩色鼠標
    any_hovered = False  # 如果你有按鈕物件，可以在這裡傳入偵測 (例如：btn.is_hovered)
    ...
    # 3. 最終等比例投射：將 1024x768 虛擬畫布無損縮放至玩家設定的 screen 實際尺寸
    screen.blit(pygame.transform.scale(canvas, (WIDTH, HEIGHT)), (0, 0))
    

    # 取得滑鼠在 1024x768 虛擬畫布中的邏輯座標
    mx, my = pygame.mouse.get_pos()
    logical_mx = int(mx * (1024 / WIDTH))
    logical_my = int(my * (768 / HEIGHT))
    ...
    any_hovered = False  # 這裡可以根據你按鈕的 hover 狀態去偵測
    
    current_cursor_pos = cursor_manager.update((logical_mx, logical_my), any_hovered, is_clicking)
    cursor_manager.draw(canvas, current_cursor_pos)

    # 最終等比例投射
    canvas.blit(pygame.transform.scale(canvas, (WIDTH, HEIGHT)), (0, 0))
