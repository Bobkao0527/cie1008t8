...
def assetsload(WIDTH,HEIGHT):
    ...
    Asset_dict = {
        ...
        "b049gaming0-21":pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","animdict","b049gaming0-2.png")).convert_alpha(), (int(WIDTH/3), int(WIDTH/3))),
        "b049gaming0-22":pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","animdict","b049gaming0-2.png")).convert_alpha(), (int(WIDTH/9), int(WIDTH/9))),
        "b049gaming0-23":pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","animdict","b049gaming0-2.png")).convert_alpha(), (int(WIDTH/27), int(WIDTH/27))),
...
while running:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if Background == "Battle" and cheat_mode:
                if event.key == pygame.K_1:
                    dice_value = 1
                    cheat_mode = False
                    attackted = False  # 老千出千成功視為拿到新骰子，解鎖攻擊
    ...
    elif Background == "Gaming": #這裡要改，闖關區
        screen.blit(Asset_dict[f'b049gaming{Gamekind}'], (0, 0))
        if Gamekind == 0:
            screen.blit(Asset_dict[f'b049gaming0-21'], (WIDTH/2+WIDTH*0.3*math.sin(Global_Time/FPS),0.6*HEIGHT))
            screen.blit(Asset_dict[f'b049gaming0-22'], (WIDTH/2-WIDTH*0.15*math.sin(Global_Time/FPS),0.5*HEIGHT))
            screen.blit(Asset_dict[f'b049gaming0-23'], (WIDTH/2+WIDTH*0.05*math.cos(Global_Time/FPS),0.4*HEIGHT))
            screen.blit(Asset_dict[f'b049gaming0-1'], (pygame.mouse.get_pos()[0]-WIDTH/2,max(pygame.mouse.get_pos()[1]-HEIGHT/2,0)))
    elif Background == "Betting": #這裡要改，零和遊戲
        screen.blit(Asset_dict[f'b049gaming{Gamekind}'], (0, 0))
