...
Pr,Pc = 11,11
PlayerV = 5.0
Background = "TitleMenu" #這個是到時候切背景可以改的東東
Global_Time = 0 #系統級時間碼
...
title_story_rect = None
#戰鬥
player_hp = 30
player_max_hp = max(player_hp, 30)
enemy_hp = 600
leak_layer = 0
enemy_max_hp = 600
battle_round = 1
...
bgm_playlist = [
    os.path.join("assets", "bgm", "god_of_war.mp3"),
    os.path.join("assets", "bgm", "mango_jump.mp3"),
    os.path.join("assets", "bgm", "jiu_wan_18_guai.mp3")
]
current_bgm_index = 0
...
def play_next_bgm():
    ...
    if bgm_playlist:
        try:
            pygame.mixer.music.load(bgm_playlist[current_bgm_index])
            pygame.mixer.music.play(0) # 0 代表播完一次就停止，用來觸發 MUSIC_END_EVENT
            current_bgm_index = (current_bgm_index + 1) % len(bgm_playlist) # 播完後索引換下一首
...
def assetsload(WIDTH,HEIGHT):
    ...
    EventCardImage = {
        ...
        "賭狗": pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","card","gunskill1.jpg")).convert_alpha(), (EVENT_CARD_W, EVENT_CARD_H)),
        "顆秒": pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","card","gunskill2.jpg")).convert_alpha(), (EVENT_CARD_W, EVENT_CARD_H)),
        "雙槍": pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","card","gunskill3.png")).convert_alpha(), (EVENT_CARD_W, EVENT_CARD_H)),
        "外圈刮" :pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","card","hammerskill1.png")).convert_alpha(), (EVENT_CARD_W, EVENT_CARD_H)),
        "流血":pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","card","hammerskill2.jpg")).convert_alpha(), (EVENT_CARD_W, EVENT_CARD_H)),
        "諾克薩斯斷頭台":pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","card","hammmerskill3.jpg")).convert_alpha(), (EVENT_CARD_W, EVENT_CARD_H)),
        "撕裂傷(被動)":pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","card","whipskill1.jpg")).convert_alpha(), (EVENT_CARD_W, EVENT_CARD_H)),
        "纏繞(被動)":pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","card","whipskill2.png")).convert_alpha(), (EVENT_CARD_W, EVENT_CARD_H)),
        "鞭刑(主動)":pygame.transform.smoothscale(pygame.image.load(os.path.join("assets","card","whipskill3.png")).convert_alpha(), (EVENT_CARD_W, EVENT_CARD_H))
    }
    #字體載入
...
def draw_text_End(text, x, y, color=(255, 255, 255)):
    img = pygame.font.Font(FHeadPath, int(HEIGHT / 5)).render(text, True, color)
    imgr = img.get_rect()
    imgr.center = (x,y)
    screen.blit(img, imgr)

def draw_text_story(text, x, y, color=(255, 255, 255)):
    img = pygame.font.Font(FHeadPath, int(HEIGHT / 45)).render(text, True, color)
    imgr = img.get_rect()
    imgr.center = (x,y)
    screen.blit(img, imgr)

def draw_menu_button(text, center_x, center_y, w, h, mouse_pos):
    rect = pygame.Rect(0, 0, w, h)
    rect.center = (center_x, center_y)

    if rect.collidepoint(mouse_pos):
        fill_color = (80, 190, 110)
        border_color = (255, 245, 180)
    else:
        fill_color = (35, 120, 70)
        border_color = (220, 180, 90)

    shadow_rect = rect.copy()
    shadow_rect.x += int(WIDTH * 0.008)
    shadow_rect.y += int(HEIGHT * 0.008)

    pygame.draw.rect(screen, (20, 10, 5), shadow_rect, border_radius=18)
    pygame.draw.rect(screen, fill_color, rect, border_radius=18)
    pygame.draw.rect(screen, border_color, rect, 5, border_radius=18)

    inner_rect = rect.inflate(-int(w * 0.08), -int(h * 0.35))
    pygame.draw.rect(screen, (160, 230, 140), inner_rect, 2, border_radius=10)

    label_shadow = font_mid.render(text, True, (20, 20, 20))
    label = font_mid.render(text, True, (255, 255, 255))

    label_rect = label.get_rect(center=rect.center)
    shadow_label_rect = label_rect.copy()
    shadow_label_rect.x += 3
    shadow_label_rect.y += 3

    screen.blit(label_shadow, shadow_label_rect)
...
def weapon_attack(card_type):
    ...
    global seal_monster_once, boss_no_immunity, next_dice_fixed_three
    ...
    if card_type == "gun":
        ...
        if caning_bonus:
                if leak_layer > 0:
                    caning_damage = max(int(enemy_max_hp*0.25), 1.0)
                    seal_monster_once = True
                else:
                    caning_damage = 0
        ...
        if  sum(dice_values) >= 15:
            if MAP[Pr, Pc] == 9:
                ...
                hook_damage = max(enemy_hp*0.25, 1)
                enemy_hp -= int((hook_damage + damage)*(max(1.5*hammer1_layer,1)))
    ...
    if sum(dice_values) >= 20 and hammer2_bonus:
        player_hp += int(player_max_hp * 0.2)
...
def reset_battle():
    ...
    if MAP[Pr, Pc] == 9:
        original_enemy_max_hp = 900 + 300 * GameRound
...
def reset_game():
    ...
    global mango_bonus, next_battle_cc, bgm_enabled
...
def Animation():
    ...
    if 'AnimEnd' in Anim_Time and Anim_Time.get('AnimEnd') > 0:
        AnimEnd()
    if 'AnimChangeLevel' in Anim_Time and Anim_Time.get('AnimChangeLevel') > 0:
        AnimChangeLevel()
...
def AnimChangeLevel():
    global Background
    global Asset_dict
    tick = 120 - Anim_Time['AnimChangeLevel']
    br = Asset_dict["GateG"]
    rbr = pygame.transform.rotate(pygame.transform.scale(br,(max(WIDTH * tick / 60 + 1,1),max(WIDTH * tick / 60 + 1,1))), tick*5)
    recr = rbr.get_rect()
    recr.center = (WIDTH // 2, HEIGHT // 2)
...
running = True
while running:
    for event in pygame.event.get(): #事件區(鍵盤滑鼠輸入)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN: #滑鼠點擊判定區
            ...
            if event.button == 1:
                ...
                elif Background == "TitleMenu":
                    if title_start_rect is not None and title_start_rect.collidepoint(mouse_pos):
                        Background = "StartWeapon"
                        Global_Time = 0

                    elif title_story_rect is not None and title_story_rect.collidepoint(mouse_pos):
                        Background = "Story"

                elif Background == "Story":
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_m :
                if Background == "Battle" and enemy_hp <= 0 :
                    Background = "Map"
                    attackted = False
                if MAP[Pr, Pc] == 9 and GameRound < 1 and mapused == True: ######測試用
                    GameRound += 1
                    ...
                    gates_group = pygame.sprite.Group()
                    refresh_gates(Pr, Pc, gates_group)
                elif MAP[Pr, Pc] == 9 and GameRound == 1 and mapused == True and End == False:
                    print("已完成所有輪數，無法繼續進行！")
                    End = True
                    FinishTime = Global_Time
    ...
    #顯示區
    if Background == "TitleMenu":
        screen.blit(choose_weapon_back, (0, 0))

        Allblack.set_alpha(90)
        screen.blit(Allblack, (0, 0))

        mouse_pos = pygame.mouse.get_pos()

        title_font = pygame.font.Font(
            os.path.join("assets", "font", "NotoSansTC-Bold.ttf"),
            int(WIDTH / 13)
        )

        sub_font = pygame.font.Font(
            os.path.join("assets", "font", "NotoSansTC-Regular.ttf"),
            int(WIDTH / 38)
        )

        title_shadow = title_font.render("骰子迷因大亂鬥", True, (40, 20, 10))
        title_img = title_font.render("骰子迷因大亂鬥", True, (255, 210, 80))

        title_rect = title_img.get_rect(center=(int(WIDTH * 0.5), int(HEIGHT * 0.20)))
        ...
        subtitle = sub_font.render("A Meme Dice Dungeon Adventure", True, (255, 255, 255))
        subtitle_rect = subtitle.get_rect(center=(int(WIDTH * 0.5), int(HEIGHT * 0.31)))
        screen.blit(subtitle, subtitle_rect)

        panel_rect = pygame.Rect(0, 0, int(WIDTH * 0.34), int(HEIGHT * 0.34))
        panel_rect.center = (int(WIDTH * 0.5), int(HEIGHT * 0.58))

        pygame.draw.rect(screen, (40, 25, 15), panel_rect, border_radius=20)
        pygame.draw.rect(screen, (210, 150, 70), panel_rect, 5, border_radius=20)

        title_start_rect = draw_menu_button(
            "[ 開始遊戲 ]",
            int(WIDTH * 0.5),
            int(HEIGHT * 0.52),
            int(WIDTH * 0.26),
        ...
        title_story_rect = draw_menu_button(
            "[ 背景故事 ]",
            int(WIDTH * 0.5),
            int(HEIGHT * 0.64),
            int(WIDTH * 0.26),
        ...
        Allblack.set_alpha(170)
        screen.blit(Allblack, (0, 0))

        story_panel = pygame.Rect(0, 0, int(WIDTH * 0.70), int(HEIGHT * 0.62))
        story_panel.center = (int(WIDTH * 0.5), int(HEIGHT * 0.50))

        pygame.draw.rect(screen, (35, 22, 15), story_panel, border_radius=25)
        pygame.draw.rect(screen, (220, 170, 80), story_panel, 5, border_radius=25)
        ...
        story_lines = [
            "2029 年，台灣網路世界因為過量迷因污染，產生了名為「次元裂縫」的現象。",
            "每當聊天室、實況精華、迷因影片與爭議事件累積到一定濃度，就會形成新的異界。",
            "而其中最危險的裂縫——就是傳說中的：「館域」",
            "擊敗菁英怪，挑戰 Boss，走到迷因地城的最深處吧。",
            "在這個世界裡，所有網路迷因都被具現化。",
            "實況主的怒吼能變成技能。聊天室的洗版能形成詛咒。 精華影片會扭曲現實。",
            "甚至連一句：「哭啊！」都足以震碎敵人的護甲。",
            "你是一名「聊天室獵人」，專門進入各種迷因次元，回收失控的網路能量。",
            "某一天，你在一個網路論壇知道了館域的存在，知道了進入館域的方法，",
            "當你做好一切準備後，點開了之前做好的館域的連結，同時， 你的螢幕突然開始瘋狂跳針。",
            "突然迸出的聊天室開始洗版：77777777 好了拉",
            "下一秒，世界崩壞。你被吸入「館域」。"
        ]
        ...
        draw_text(
            ...
            (255, 255, 255))
        draw_text(
            f"通關時間：{FinishTime//FPS//60}分{(FinishTime//FPS)%60}秒",
            int(WIDTH * 0.5),
            int(HEIGHT * 0.6),
            font_mid,
            (255, 255, 255))
        draw_text(
            ...
            font_mid,
            (255, 255, 255))
        if FinishTime < 3 * 60 * FPS:
            draw_text_End(
                "S",
                int(WIDTH * 0.5),
                int(HEIGHT * 0.45),
                (255, 215, 0))
        elif FinishTime < 5 * 60 * FPS:
            draw_text_End(
                "A",
                int(WIDTH * 0.5),
                int(HEIGHT * 0.45),
                (255, 0, 255))
        elif FinishTime < 8 * 60 * FPS:
            draw_text_End(
                "B",
                int(WIDTH * 0.5),
                int(HEIGHT * 0.45),
                (148, 0, 211))
        elif FinishTime < 10 * 60 * FPS:
            draw_text_End(
                "C",
                int(WIDTH * 0.5),
                int(HEIGHT * 0.45),
