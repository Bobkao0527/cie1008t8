...
while running:
    ...
    if Background == "Map":
        ...
        if hit_gate and mapused:
            ...
            if MAP[Pr,Pc] == 0 and RoomStep not in [4,7+GameRound,8+GameRound]:
                MAP[Pr,Pc] = random.randint(3,3) #傳送瞬間去改 Numpy 陣列，隨機判定房間類型，這裡不處理精英怪、整備、Boss，這幾個交給RoomStep處理 (初始值2,6)
            elif RoomStep == 4:
                MAP[Pr,Pc] = 7
    ...
    if Background == "StartWeapon":
        ...
        draw_text(f'{coin-now_coin}',int(WIDTH * 0.5),int(HEIGHT * 0.75),font_big,(255, 255, 255))
