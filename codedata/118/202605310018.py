...
special_card = special_card = [{"name": "賭狗", "type": "gambler"},{"name": "顆秒", "type": "gun_kill_quick"},{"name": "雙槍", "type": "double_gun"},{"name": "外圈刮", "type": "hammer_out"},{"name": "流血", "type": "hammer_bleed"},{"name": "諾克薩斯斷頭台", "type": "hammer_execute"},{"name": "撕裂傷(被動)", "type": "whip_bleed"},{"name": "纏繞(被動)", "type": "whip_bind"},{"name": "鞭刑(主動)", "type": "whip_active"}]
SelectedWeapon = None
...
def apply_event_card(event_card):
    ...
    if event_type == "monster_double_attack":
        ...
        battle_message = "事件:下場戰鬥敵方血量x2，玩家攻擊減半"
...
def generate_shop_cars():
    ...
    shop_positions = []
    shop_room = (Pr, Pc)
    if len(special_card)>= 2:
        shop_card = random.sample(special_card, 2)
    else:
        shop_card = special_card.copy()
...
def reset_game():
    ...
    global shop_card, shop_positions, shop_room, coin, card_price
...
while running:
    ...
    if Background == "StartWeapon":
        screen.blit(Asset_dict["choose_weapon_back"], (0,0))
        ...
        if Gamekind == 0:
            ...
            draw_text(f'{(GameTimer-Global_Time)/FPS:.2f}',int(WIDTH * 0.05),int(HEIGHT * 0.05),font_mid,(255, 255, 255))
        ...
        draw_text(f'{coin-now_coin}',int(WIDTH * 0.48),int(HEIGHT * 0.67),font_big,(255, 255, 255))
