...
bgm_list = ["mango_jump.mp3", "jiu_wan_18_guai.mp3", "god_of_war.mp3"]
current_bgm_index = 0
...
def apply_event_card(event_card):
    ...
    global current_bgm_index, bgm_list, MUSIC_END
    ...
    if event_type == "monster_double_attack":
        ...
        try:
            ...
            current_bgm_index = 0
            bgm_path = os.path.join("assets", "bgm", bgm_list[current_bgm_index]) 
            pygame.mixer.music.load(bgm_path)
            pygame.mixer.music.set_endevent(MUSIC_END)
            pygame.mixer.music.play()
            print(f"播放芒果醬 BGM：{bgm_list[current_bgm_index]}")
...
def buy_card(card):
    ...
    if coin >= card_price:
        ...
        elif name == "外圈刮": hammer2_bonus = True
        elif name == "流血": hammer1_bonus = True
            ...
            if event.button == 1:
                if Background == "X_Ingame_Settings":
                    ...
                    if bgm_rect.collidepoint(mouse_pos):
                        ...
                        if bgm_enabled:
                            try:
                                random.shuffle(bgm_list) # 打亂清單
                                current_bgm_index = 0
                                bgm_path = os.path.join("assets", "bgm", bgm_list[current_bgm_index]) 
                                pygame.mixer.music.load(bgm_path)
                                pygame.mixer.music.set_endevent(MUSIC_END) # 設定結束事件
                                pygame.mixer.music.play()
                                print(f"BGM 開啟，已獲得芒果醬攻擊加成！目前播放：{bgm_list[current_bgm_index]}")
                            except Exception as e:
                                print(f"無法播放 BGM: {e}")
                        else:
                            if pygame.mixer.get_init():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_m :
                if Background == "Battle" and enemy_hp <= 0 :
                    Background = "Map"
                    attackted = False
                if MAP[Pr, Pc] == 9 and GameRound < 1 and mapused == True:
                    GameRound += 1
